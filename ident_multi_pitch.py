...
def plot_nsga2_analysis(result_nsga, problem, history=None, param_names=None, results_dir="results"):
    """
    Gera e salva plots de análise do NSGA-II na pasta results.
    - Frente de Pareto
    - Convergência dos objetivos
    - Boxplot dos parâmetros das soluções não dominadas
    - (Opcional) Histórico da frente de Pareto
    """
    ...
    if F is not None:
        ...
        ax.set_title('Convergência dos Objetivos (menores valores)')
        ax.set_xlabel('Soluções ordenadas')
        ax.set_ylabel('Valor do objetivo')
        ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, "convergencia_objetivos.png"))
        plt.close()

    # 3. Boxplot dos parâmetros das soluções não dominadas
    if hasattr(result_nsga, 'X'):
        ...
        ax.set_title('Boxplot dos Parâmetros (Soluções Não Dominadas)')
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, "boxplot_parametros.png"))
        plt.close()

    # 4. Histórico da frente de Pareto (se fornecido)
    if history is not None:
        for i, hist in enumerate(history):
            ...
            try:
                ...
                fig.title(f"Pareto Front - Geração {i+1}")
...
if __name__ == "__main__":
    ...
    # Plots de análise NSGA-II
    param_names = ["pitch_K", "pitch_omega", "pitch_zeta", "pitch_max", "Cx"]
    plot_nsga2_analysis(result_nsga, problem, param_names=param_names)
